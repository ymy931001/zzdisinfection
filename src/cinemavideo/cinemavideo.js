...
class App extends React.Component {
  ...
  showhistory = (text, record, index) => {
    ...
    this.setState({
      ...
      decendingdatas: JSON.parse(record.timepairs),
      ...
      if (this.state.decendingdatas != null) {
        if (this.state.decendingdatas.length != 0 && this.state.decendingdatas != null) { //eslint-disable-line
          var newarr = []
          for (var j in this.state.decendingdatas) {
            var num3 = moment(this.state.decendingdatas[j].start).format('HH:mm:ss')
            var num4 = moment(this.state.decendingdatas[j].end).format('HH:mm:ss')
            newarr.push({
              ...
              'time': moment(this.state.decendingdatas[j].start).format('HH:mm:ss') + " ~ " + moment(this.state.decendingdatas[j].end).format('HH:mm:ss')
            })
          }
          console.log(newarr)
          this.setState({
      ...
      <Layout style={{ minHeight: "100vh" }} id="videodata">
        <Layout>
          <Content style={{ margin: "16px 16px" }} id="NodeManage">
            <Card title="影院消毒--监测列表" id="nodeManage" headStyle={{ fontWeight: 'bold', fontSize: '18px' }}>
